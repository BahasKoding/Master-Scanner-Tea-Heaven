...
class HistorySaleSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('🚀 Memulai konversi data dari ketiga file excel...');
        ...
        // 3. PROSES FILE EXCEL MAY 26 MORNING (file_excel_may26_morning.md) - 38 data dengan offset ID
        $this->processMay26MorningFile();

        $totalRecords = HistorySale::count();
        $this->command->info("✅ SEEDER SELESAI! Total {$totalRecords} riwayat penjualan berhasil dibuat dari ketiga file.");
        $this->command->info("📊 Kombinasi: file_excel.md (2780 data) + file_excel_terbaru.md (715 data) + file_excel_may26_morning.md (38 data)");
    }
    ...
    /**
     * Process file_excel_may26_morning.md (new file with 38 records)
     */
    private function processMay26MorningFile()
    {
        $this->command->info('📂 Memproses file_excel_may26_morning.md (38 data)...');

        $filePath = base_path('file_excel_may26_morning.md');
        ...
        if (empty($lines)) {
            $this->command->error('File excel may 26 morning kosong!');
            return;
        }
        ...
        $batchSize = 100;
        $totalProcessed = 0;
        $idOffset = 20000; // Offset yang lebih besar untuk menghindari konflik dengan data sebelumnya

        $this->command->info('Memproses ' . count($lines) . ' baris dari file excel may 26 morning...');
        ...
        foreach ($lines as $index => $line) {
            try {
                ...
                if (count($salesData) >= $batchSize) {
                    try {
                        HistorySale::insert($salesData);
                        $this->command->info("✓ Batch " . ceil($totalProcessed / $batchSize) . " file may 26 morning ({$batchSize} records)");
                        $salesData = []; // Reset array
                    } catch (\Exception $e) {
                        $this->command->error("Gagal menyimpan batch file may 26 morning: " . $e->getMessage());
                        $salesData = [];
                    }
                }
            } catch (\Exception $e) {
                $this->command->warn("Error parsing baris " . ($index + 2) . " file may 26 morning: " . $e->getMessage());
                continue;
            }
        }
        ...
        if (!empty($salesData)) {
            try {
                HistorySale::insert($salesData);
                $this->command->info("✓ Batch terakhir file may 26 morning (" . count($salesData) . " records)");
            } catch (\Exception $e) {
                $this->command->error("Gagal menyimpan batch terakhir file may 26 morning: " . $e->getMessage());
            }
        }

        $this->command->info("✅ File excel may 26 morning selesai diproses! ({$totalProcessed} baris)");
    }
}
