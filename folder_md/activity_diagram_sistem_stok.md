...
## 10. Alur Stock Opname

```
[Start] --> "Pengguna memulai sesi stock opname baru"
        --> "StockOpnameController.create() dipanggil"
        --> "Buat record StockOpname dengan status 'draft'"
        --> "Generate unique session ID"
        --> "Tampilkan form input stock opname"
        
[Input] --> "Pengguna input data aktual stock"
        --> "StockOpnameController.store() dipanggil"
        --> "StockOpnameService.processOpnameData() dipanggil"
        --> "Untuk setiap item yang diinput:"
            --> "Hitung variance (selisih stock sistem vs aktual)"
            --> "Buat StockOpnameDetail record"
            --> "Tandai item yang memiliki variance"
        
[Review] --> "Supervisor review variances"
        --> [Decision] "Ada variance yang perlu adjustment?"
            --> [Ya] "Tandai items untuk adjustment"
            --> "Input alasan adjustment"
            --> "StockOpnameController.approve() dipanggil"
            --> "Update stock di sistem"
            --> "Catat history adjustment"
            --> [End]
            
            [Decision] --> [Tidak] "Finalisasi stock opname"
            --> "Set status = 'completed'"
            --> "Generate laporan final"
            --> [End]
```

## 11. Alur Adjustment Stock Opname

```
[Start] --> "Supervisor approve adjustment"
        --> "StockOpnameService.processAdjustment() dipanggil"
        --> "Untuk setiap item yang diapprove:"
            --> [Decision] "Tipe item?"
                --> [Finished Goods] "Update FinishedGoods stock"
                --> "Catat adjustment di history"
                --> [End]
                
                [Decision] --> [Bahan Baku] "Update InventoryBahanBaku"
                --> "Catat adjustment di history"
                --> [End]
```
...
Untuk mengintegrasikan sistem Stock Opname, pastikan komponen berikut telah diimplementasi:

1. **Database Tables**
   - `stock_opname` - Header sesi opname
   - `stock_opname_details` - Detail item yang diopname
   - `stock_opname_adjustments` - History adjustment

2. **Models**
   - `StockOpname` dengan relasi ke details dan adjustments
   - `StockOpnameDetail` dengan relasi ke products/bahan baku
   - `StockOpnameAdjustment` untuk tracking changes

3. **Services**
   - `StockOpnameService` untuk business logic
   - Integration dengan `StockService` yang ada

4. **Permissions**
   - `create-opname` - Mulai sesi baru
   - `process-opname` - Input data
   - `approve-opname` - Approve adjustments
   - `view-opname-report` - Lihat laporan

5. **Mobile Integration**
   - Interface mobile-friendly untuk input data
...
Note: Stock Opname sebaiknya dilakukan secara rutin (minimal bulanan) untuk menjaga akurasi data stock di sistem.
